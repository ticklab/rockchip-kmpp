    ...
    if (change) {
        ...
        if (change & MPP_ENC_JPEG_CFG_CHANGE_QP) {
            ...
            if (dst->quant < 0 || dst->quant > 10) {
                mpp_err_f("invalid quality level is not in range [0..10] set to default 8\n");
                dst->quant = 8;
            }
...
static void jpege_proc_show(void *seq_file, void *ctx, RK_S32 chl_id)
{
    struct seq_file *seq  = (struct seq_file *)seq_file;
    JpegeCtx *p = (JpegeCtx *)ctx;
    MppEncCfgSet *cfg = p->cfg;
    MppEncPrepCfg *prep = &cfg->prep;
    MppEncJpegCfg  *jpege = &cfg->codec.jpeg;

    seq_puts(seq, "\n--------jpege chn attr----------------------------------------------------------------------------\n");
    seq_printf(seq, "%7s%10s%10s%10s%10s%10s%10s\n", "ID", "Width", "Height", "quant", "qfactor", "qfmax", "qfmin");
    seq_printf(seq, "%7d%10u%10u%10u%10u%10u%10u\n", chl_id, prep->width, prep->height, jpege->quant, jpege->q_factor,
