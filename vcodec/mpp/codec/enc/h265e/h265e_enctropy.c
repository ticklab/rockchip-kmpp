...
const RK_U8 g_lpsTable[64][4] = { 
 ...
 const RK_U8 g_nextState[128][2] = { 
    ...
    {18, 12}, {13, 19}, {20, 14}, {15, 21}, {22, 16}, {17, 23}, {24, 18}, {19,
									   25},
    {26, 18}, {19, 27}, {28, 22}, {23, 29}, {30, 22}, {23, 31}, {32, 24}, {25,
									    33},
    {34, 26}, {27, 35}, {36, 26}, {27, 37}, {38, 30}, {31, 39}, {40, 30}, {31,
									    41},
    {42, 32}, {33, 43}, {44, 32}, {33, 45}, {46, 36}, {37, 47}, {48, 36}, {37,
									    49},
    {50, 38}, {39, 51}, {52, 38}, {39, 53}, {54, 42}, {43, 55}, {56, 42}, {43,
									    57},
    {58, 44}, {45, 59}, {60, 44}, {45, 61}, {62, 46}, {47, 63}, {64, 48}, {49,
									    65},
    {66, 48}, {49, 67}, {68, 50}, {51, 69}, {70, 52}, {53, 71}, {72, 52}, {53,
									    73},
    {74, 54}, {55, 75}, {76, 54}, {55, 77}, {78, 56}, {57, 79}, {80, 58}, {59,
									    81},
    {82, 58}, {59, 83}, {84, 60}, {61, 85}, {86, 60}, {61, 87}, {88, 60}, {61,
									    89},
    {90, 62}, {63, 91}, {92, 64}, {65, 93}, {94, 64}, {65, 95}, {96, 66}, {67,
									    97},
    {98, 66}, {67, 99}, {100, 66}, {67, 101}, {102, 68}, {69, 103}, {104, 68},
    {69, 105}, {106, 70}, {71, 107}, {108, 70}, {71, 109}, {110, 70}, {71,
									111},
    {112, 72}, {73, 113}, {114, 72}, {73, 115}, {116, 72}, {73, 117}, {118,
									74},
    {75, 119}, {120, 74}, {75, 121}, {122, 74}, {75, 123}, {124, 76}, {77,
									125},
    {124, 76}, {77, 125}, {126, 126}, {127, 127} 
};
 ...
 RK_U8 sbacInit(RK_S32 qp, RK_S32 initValue) 
{
	...
	RK_S32 initState =
	    MPP_MIN(MPP_MAX(1, (((slope * qp) >> 4) + offset)), 126);
	RK_U32 mpState = (initState >= 64);
	RK_U8 m_state =
	    ((mpState ? (initState - 64) : (63 - initState)) << 1) + mpState;
	 return m_state;
}

 static void initBuffer(h265_contex_model * contextModel, SliceType sliceType,
			  RK_U8 cabacIntFlag, RK_S32 qp, RK_U8 * ctxModel,
			  RK_S32 size) 
{
	RK_S8 initType = 0;
	RK_S32 n = 0;
...
{
	h265_slice * slice = (h265_slice *) slice_ctx;
	RK_U8 cabacInitFlag = slice->cabac_init_flag;
	RK_S32 qp = slice->slice_qp;
	SliceType sliceType = slice->slice_type;
	 h265e_dbg_func("enter\n");
	initBuffer(&slice->contex_models[OFF_SPLIT_FLAG_CTX], sliceType,
		    cabacInitFlag, qp, (RK_U8 *) INIT_SPLIT_FLAG,
		    NUM_SPLIT_FLAG_CTX);
	initBuffer(&slice->contex_models[OFF_SKIP_FLAG_CTX], sliceType,
		    cabacInitFlag, qp, (RK_U8 *) INIT_SKIP_FLAG,
		    NUM_SKIP_FLAG_CTX);
	initBuffer(&slice->contex_models[OFF_MERGE_FLAG_EXT_CTX], sliceType,
		    cabacInitFlag, qp, (RK_U8 *) INIT_MERGE_FLAG_EXT,
		    NUM_MERGE_FLAG_EXT_CTX);
	initBuffer(&slice->contex_models[OFF_MERGE_IDX_EXT_CTX], sliceType,
		    cabacInitFlag, qp, (uint8_t *) INIT_MERGE_IDX_EXT,
...
}  void h265e_cabac_start(h265_cabac_ctx * cabac_ctx) 
{
	...
	cabac_ctx->m_numBufferedBytes = 0;
	cabac_ctx->m_bufferedByte = 0xff;
}  void h265e_cabac_init(h265_cabac_ctx * cabac_ctx, MppWriteCtx * bitIf) 
{
	h265e_dbg_func("enter\n");
	cabac_ctx->m_bitIf = bitIf;
	h265e_cabac_start(cabac_ctx);
	h265e_dbg_func("leave\n");
}  void h265e_cabac_writeOut(h265_cabac_ctx * cabac_ctx) 
{
...
void h265e_cabac_encodeBin(h265_cabac_ctx * cabac_ctx,
			   h265_contex_model * ctxModel, RK_U32 binValue) 
{
	...
	range = cabac_ctx->m_range;
	state = sbacGetState(mstate);
	lps = g_lpsTable[state][((uint8_t) range >> 6)];
	range -= lps;
	 
	    //  mpp_log("ctxModel.m_state  %d binValue %d \n", ctxModel->m_state, binValue);
	    //  assert(lps >= 2);
	    numBits = (RK_U32) (range - 256) >> 31;
	low = cabac_ctx->m_low;
 ...
 void h265e_cabac_encodeBinTrm(h265_cabac_ctx * cabac_ctx, RK_U32 binValue) 
{
	h265e_dbg_func("enter\n");
 ...
 void h265e_cabac_finish(h265_cabac_ctx * cabac_ctx) 
{
	...
	mpp_writer_put_bits(s, cabac_ctx->m_low >> 8,
			     13 + cabac_ctx->m_bitsLeft);
	 h265e_dbg_func("leave\n");
}
