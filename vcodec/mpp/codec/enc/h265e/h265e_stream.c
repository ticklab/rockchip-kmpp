...
MPP_RET h265e_stream_write1_with_log(H265eStream * s, RK_U32 val, char *name)
{
...
MPP_RET h265e_stream_write_ue_with_log(H265eStream * s, RK_U32 val, char *name)
{

	h265e_dbg(H265E_DBG_HEADER,
		  "write UE bits name %s, val %d ", name, val);
#if 1
	{
		RK_S32 size = 0;
		RK_S32 tmp = ++val;
...
MPP_RET h265e_stream_write_se_with_log(H265eStream * s, RK_S32 val, char *name)
{
	...
	h265e_dbg(H265E_DBG_HEADER,
		  "write SE bits name %s, val %d ", name, val);

	return h265e_stream_write_with_log(s, val, size, name);;
}

MPP_RET h265e_stream_write32(H265eStream * s, RK_U32 i_bits, char *name)
{
