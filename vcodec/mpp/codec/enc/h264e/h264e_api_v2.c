...
static MPP_RET h264e_proc_prep_cfg(MppEncPrepCfg *dst, MppEncPrepCfg *src)
{
    ...
    if (change) {
        ...
        if (MPP_FRAME_FMT_IS_FBC(dst->format) && (dst->mirroring || dst->rotation)) {
            mpp_err("invalid cfg fbc data no support mirror %d or rotaion %d",
...
static void h264e_proc_show(void *seq_file, void *ctx, RK_S32 chl_id)
{
    H264eCtx *p = (H264eCtx *)ctx;
    ...
    seq_printf(seq, "%7d%10u%10u%10s\n", chl_id, prep->width, prep->height, strof_profle(MPP_VIDEO_CodingAVC,h264->profile));
    ...
    seq_printf(seq, "%7d%10s%10s%10s%8s%6d%8d%8d\n", chl_id, strof_profle(MPP_VIDEO_CodingAVC,h264->profile), strof_bool(h264->entropy_coding_mode),
