...
void h264e_dpb_build_list(H264eDpb *dpb, EncCpbStatus *cpb)
{
    ...
    for (i = 0; i < MAX_CPB_REFS; i++) {
        ...
        p = find_cpb_frame(dpb->frames, dpb->total_cnt, frm);
        if (!frm->is_lt_ref) {
            dpb->stref[st_size++] = p;
    ...
    if (st_size > 1) {
        ...
        sort(dpb->stref, st_size, sizeof(dpb->stref[0]), cmp_st_list, NULL);
    ...
    if (lt_size > 1) {
        ...
        sort(dpb->ltref, lt_size, sizeof(dpb->ltref[0]), cmp_lt_list, NULL);
...
MPP_RET h264e_dpb_proc(H264eDpb *dpb, EncCpbStatus *cpb)
{
    ...
    if (NULL == dpb->curr)
        mpp_err_f("frm %d failed to find a slot for curr \n", seq_idx);
