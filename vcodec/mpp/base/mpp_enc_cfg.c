...
static RK_S32 node_len = 10 * 1024;
...
#define ENC_CFG_SET_ACCESS(func_name, in_type, cfg_type) \
    ...
    { \
        ...
        p = (MppEncCfgImpl *)cfg; \
        info = mpp_trie_get_info(p->api, name); \
        if (NULL == info) { \
            mpp_err_f("failed to set %s to \n", name); \
            return MPP_NOK; \
        } \
        api = (MppCfgApi *)info; \
        ...
        ret = ((CfgSet##cfg_type)api->api_set)(&p->cfg, api, val); \
        return ret; \
    }
...
#define ENC_CFG_GET_ACCESS(func_name, in_type, cfg_type) \
    ...
    { \
        ...
        ret = ((CfgGet##cfg_type)api->api_get)(&p->cfg, api, val); \
        return ret; \
    }
