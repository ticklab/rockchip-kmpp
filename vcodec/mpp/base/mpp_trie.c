...
static RK_S32 trie_get_node(MppTrieImpl * trie)
{
	...
	if (trie->node_used >= trie->node_count) {
		...
		MppTrieNode *new_nodes = mpp_malloc(MppTrieNode, new_count);
		if (NULL == new_nodes) {
		...
		trie_dbg_cnt("trie %p enlarge node %p:%d -> %p:%d\n",
			     trie, trie->nodes, trie->node_count, new_nodes,
			     new_count);
		memcpy(new_nodes, trie->nodes,
		       sizeof(MppTrieNode) * trie->node_count);
...
MPP_RET mpp_trie_add_info(MppTrie trie, const char **info)
{
	...
	if (p->info_used >= p->info_count) {
		...
		const char ***ptr = mpp_malloc(const char **, new_count);
		if (NULL == ptr) {
		...
		memcpy(p->info, ptr, p->info_count * sizeof(const char **));
		p->info = ptr;
...
const char **mpp_trie_get_info(MppTrie trie, const char *name)
{
	...
	return (idx && node
		&& node->info_id >= 0) ? p->info[node->info_id] : NULL;
}
