...
MPP_RET hal_info_init(HalInfo * ctx, MppCtxType type, MppCodingType coding)
{
	...
	MPP_RET ret = MPP_NOK;
	RK_U32 elem_nb =
		(type == MPP_CTX_DEC) ? (ENC_INFO_BUTT) : (DEC_INFO_BUTT);
	HalInfoImpl *impl =
		mpp_calloc_size(HalInfoImpl,
				sizeof(HalInfoImpl) +
...
MPP_RET hal_info_set(HalInfo ctx, RK_U32 type, RK_U32 flag, RK_U64 data)
{
	...
	case MPP_CTX_DEC: {
		if (type <= DEC_INFO_BASE || type >= DEC_INFO_BUTT) {
			mpp_err_f
			("found invalid dec info type %d [%d:%d]\n",
	...
	case MPP_CTX_ENC: {
		if (type <= ENC_INFO_BASE || type >= ENC_INFO_BUTT) {
			mpp_err_f
			("found invalid enc info type %d [%d:%d]\n",
...
RK_U64 hal_info_to_string(HalInfo ctx, RK_U32 type, void *val)
{
	...
	case MPP_CTX_ENC: {
		...
		case ENC_INFO_PROFILE: {
			...
			str =
				strof_profle(info->coding, profile);
		}
		break;
		default: {
		}
...
RK_U64 hal_info_to_float(RK_S32 num, RK_S32 denorm)
{
	...
	if (!denorm)
		...
		snprintf((void *)&ret, sizeof(ret) - 1, "%.2f",
			 (float)num / denorm);
...
MPP_RET hal_info_from_enc_cfg(HalInfo ctx, MppEncCfgSet * cfg)
{
	...
	hal_info_set(ctx, ENC_INFO_HEIGHT, CODEC_INFO_FLAG_NUMBER,
		     prep->height);
	...
	hal_info_set(ctx, ENC_INFO_BITRATE, CODEC_INFO_FLAG_NUMBER,
		     rc->bps_target);
