...
static RK_U32 lamd_satd_qp[52] = {
	...
	0x00000305, 0x00000364, 0x000003ce, 0x00000445, 0x000004cb, 0x00000562,
	0x0000060a, 0x000006c8, 0x0000079c, 0x0000088b, 0x00000996, 0x00000ac3,
	0x00000c14, 0x00000d8f, 0x00000f38, 0x00001115, 0x0000132d, 0x00001586,
	0x00001829, 0x00001b1e, 0x00001e70, 0x0000222b, 0x0000265a, 0x00002b0c,
	0x00003052, 0x0000363c, 0x00003ce1, 0x00004455, 0x00004cb4, 0x00005618,
	0x000060a3, 0x00006c79, 0x000079c2, 0x000088ab, 0x00009967, 0x0000ac30,
	0x0000c147, 0x0000d8f2, 0x0000f383, 0x00011155, 0x000132ce, 0x00015861,
...
static void vepu580_h265_set_me_ram(H265eSyntax_new * syn,
				    hevc_vepu580_base * regs, RK_U32 index)
{
	...
	RK_U32 pic_wd64 =
		((regs->reg0196_enc_rsl.pic_wd8_m1 + 1) * 8 + 63) / 64;
	...
	if (!syn->pp.tiles_enabled_flag) {
		...
		if (index == syn->pp.num_tile_columns_minus1) {
			tile_ctu_endx =
				((regs->reg0196_enc_rsl.pic_wd8_m1 + 1) * 8 +
				 63) / 64 - 1;
		} else {
			tile_ctu_endx =
				(index +
	...
	regs->reg0222_me_cach.cme_linebuf_w = (pic_w ? pic_w : 64) / 64;
	{
		...
		if ((x_gmv % 16 - srch_w % 16) < 0) {
			cur_srch_16_w =
				(16 + (x_gmv % 16 - srch_w % 16) % 16 + srch_w * 2 +
				 15) / 16 + 1;
		} else {
			cur_srch_16_w =
				((x_gmv % 16 - srch_w % 16) % 16 + srch_w * 2 +
				 15) / 16 + 1;
		}
		if ((y_gmv % 4 - srch_h % 4) < 0) {
			cur_srch_4_h =
				(4 + (y_gmv % 4 - srch_h % 4) % 4 + srch_h * 2 +
				 3) / 4 + ctu_4_h;
		} else {
			cur_srch_4_h =
				((y_gmv % 4 - srch_h % 4) % 4 + srch_h * 2 +
				 3) / 4 + ctu_4_h;
		}
		...
		if (cime_rama_max <
		    ((cime_rama_h -
		      ctu_4_h) * regs->reg0222_me_cach.cme_linebuf_w * 4 +
		     (ramb_h * 4 * cur_srch_16_w)))
			cime_rama_h = cime_rama_h - ctu_4_h;
...

		{
			...
			regs->reg0222_me_cach.cme_rama_max =
				ram_col_h * regs->reg0222_me_cach.cme_linebuf_w +
				cur_srch_16_w;
		}
	...
	("cime_rama_h %d, cime_rama_max %d, cime_linebuf_w %d",
	 regs->reg0222_me_cach.cme_rama_h,
	 regs->reg0222_me_cach.cme_rama_max,
...
static void vepu580_h265_global_cfg_set(H265eV580HalContext * ctx,
					H265eV580RegSet * regs)
{
	...
	memcpy(&reg_wgt->iprd_wgt_qp_hevc_0_51[0], lamd_satd_qp,
	       sizeof(lamd_satd_qp));
	...
	memcpy(&reg_wgt->lvl32_intra_CST_THD0, lvl32_intra_cst_thd,
	       sizeof(lvl32_intra_cst_thd));
	memcpy(&reg_wgt->lvl16_intra_CST_THD0, lvl16_intra_cst_thd,
	       sizeof(lvl16_intra_cst_thd));
	memcpy(&reg_wgt->lvl32_intra_CST_WGT0, lvl32_intra_cst_wgt,
	       sizeof(lvl32_intra_cst_wgt));
	memcpy(&reg_wgt->lvl16_intra_CST_WGT0, lvl16_intra_cst_wgt,
...
static MPP_RET
vepu580_h265_uv_address(hevc_vepu580_base * reg_base, H265eSyntax_new * syn,
...
static MPP_RET vepu580_h265_set_roi_regs(H265eV580HalContext * ctx,
...
static MPP_RET vepu580_h265_set_rc_regs(H265eV580HalContext * ctx,
					H265eV580RegSet * regs,
					HalEncTask * task)
{
...
static MPP_RET vepu580_h265_set_pp_regs(H265eV580RegSet * regs,
					VepuFmtCfg * fmt,
...
static void vepu580_h265_set_slice_regs(H265eSyntax_new * syn,
					hevc_vepu580_base * regs)
{
...
static void vepu580_h265_set_ref_regs(H265eSyntax_new * syn,
				      hevc_vepu580_base * regs)
{
...
static void vepu580_h265_set_me_regs(H265eV580HalContext * ctx,
				     H265eSyntax_new * syn,
				     hevc_vepu580_base * regs)
{
...

	{
		RK_S32 swin_scope_wd16 =
			(regs->reg0220_me_rnge.cme_srch_h + 3 + 1) / 4 * 2 + 1;
		RK_S32 tmpMin =
			(regs->reg0220_me_rnge.cme_srch_v + 3) / 4 * 2 + 1;
...
void vepu580_h265_set_hw_address(H265eV580HalContext * ctx,
				 hevc_vepu580_base * regs, HalEncTask * task)
{
	...
	vepu580_h265_uv_address(regs, syn, (Vepu541Fmt) fmt->format, task);
...
MPP_RET hal_h265e_v580_gen_regs(void *hal, HalEncTask * task)
{
	...
	reg_ctl->reg0013_dtrns_cfg.dspr_otsd =
...
void hal_h265e_v580_set_uniform_tile(hevc_vepu580_base * regs,
...
MPP_RET hal_h265e_v580_start(void *hal, HalEncTask * enc_task)
{
	...
	if (title_num > MAX_TITLE_NUM) {
		mpp_log("title_num big then support %d, max %d", title_num,
			MAX_TITLE_NUM);
		return MPP_NOK;
	}
	...
	for (k = 0; k < title_num; k++) {
		...
		H265eV580StatusElem *reg_out =
			(H265eV580StatusElem *) ctx->reg_out[k];
		MppDevRegWrCfg cfg;
		MppDevRegRdCfg cfg1;
		...
		/* set input info */
		if (title_num > 1)
			hal_h265e_v580_set_uniform_tile(&hw_regs->reg_base, syn,
							k);
		if (k > 0) {
			RK_U32 offset = mpp_packet_get_length(enc_task->packet);
			offset += stream_len;
			reg_base->reg0175_adr_bsbs += offset;
		...
		if (hal_h265e_debug & HAL_H265E_DBG_CTL_REGS) {
			...
			for (i = 0; i < sizeof(hevc_vepu580_control_cfg) / 4;
			     i++) {
		...
		if (hal_h265e_debug & HAL_H265E_DBG_REGS) {
			...
			for (i = 0; i < 32; i++) {
				hal_h265e_dbg_regs
				("hw add cfg reg[%04x]: 0%08x\n", i * 4,
				 regs[i]);
			}
			regs += 32;
			for (i = 0; i < (sizeof(hevc_vepu580_base) - 128) / 4;
			     i++) {
...
static MPP_RET vepu580_h265_set_feedback(H265eV580HalContext * ctx,
					 HalEncTask * enc_task, RK_U32 index)
{
	...
	if (index == (ctx->title_num - 1)) {
		...
		if (mb4_num > 0)
			hal_rc_ret->iblk4_prop =
				((((fb->st_lvl4_intra_num +
				    fb->st_lvl8_intra_num) << 2) +
...
MPP_RET hal_h265e_v580_get_task(void *hal, HalEncTask * task)
{
	...
	if (!frm_status->reencode && mpp_frame_has_meta(task->frame)) {
		...
		mpp_meta_get_ptr(meta, KEY_OSD_DATA,
				 (void **)&ctx->osd_cfg.osd_data);
		mpp_meta_get_ptr(meta, KEY_OSD_DATA2,
				 (void **)&ctx->osd_cfg.osd_data2);
	}
#endif
...
MPP_RET hal_h265e_v580_ret_task(void *hal, HalEncTask * task)
{
	...
	ret = vepu580_h265_set_feedback(ctx, enc_task, ctx->title_num - 1);
	if (ret)
		return ret;
