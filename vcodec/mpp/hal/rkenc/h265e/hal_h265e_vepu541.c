...
static RK_U32 lamd_satd_qp[52] = {
	0x00000183, 0x000001b2, 0x000001e7, 0x00000223, 0x00000266, 0x000002b1,
	0x00000305, 0x00000364,
	0x000003ce, 0x00000445, 0x000004cb, 0x00000562, 0x0000060a, 0x000006c8,
	0x0000079c, 0x0000088b,
	0x00000996, 0x00000ac3, 0x00000c14, 0x00000d8f, 0x00000f38, 0x00001115,
	0x0000132d, 0x00001586,
	0x00001829, 0x00001b1e, 0x00001e70, 0x0000222b, 0x0000265a, 0x00002b0c,
	0x00003052, 0x0000363c,
	0x00003ce1, 0x00004455, 0x00004cb4, 0x00005618, 0x000060a3, 0x00006c79,
	0x000079c2, 0x000088ab,
	0x00009967, 0x0000ac30, 0x0000c147, 0x0000d8f2, 0x0000f383, 0x00011155,
	0x000132ce, 0x00015861,
...
static RK_U32 lamd_moda_qp[52] = {
	...
	0x00000124, 0x00000170,
	0x000001cf, 0x00000248, 0x000002df, 0x0000039f, 0x0000048f, 0x000005bf,
	0x0000073d, 0x0000091f,
	0x00000b7e, 0x00000e7a, 0x0000123d, 0x000016fb, 0x00001cf4, 0x0000247b,
	0x00002df6, 0x000039e9,
	0x000048f6, 0x00005bed, 0x000073d1, 0x000091ec, 0x0000b7d9, 0x0000e7a2,
	0x000123d7, 0x00016fb2,
	0x0001cf44, 0x000247ae, 0x0002df64, 0x00039e89, 0x00048f5c, 0x0005bec8,
	0x00073d12, 0x00091eb8,
	0x000b7d90, 0x000e7a23, 0x00123d71, 0x0016fb20, 0x001cf446, 0x00247ae1,
	0x002df640, 0x0039e88c,
...
static RK_U32 lamd_modb_qp[52] = {
	...
	0x000001c0, 0x00000224,
	0x000002c0, 0x00000380, 0x00000448, 0x00000580, 0x00000700, 0x00000890,
	0x00000b00, 0x00000e00,
	0x00001120, 0x00001600, 0x00001c00, 0x00002240, 0x00002c00, 0x00003800,
	0x00004480, 0x00005800,
	0x00007000, 0x00008900, 0x0000b000, 0x0000e000, 0x00011200, 0x00016000,
	0x0001c000, 0x00022400,
	0x0002c000, 0x00038000, 0x00044800, 0x00058000, 0x00070000, 0x00089000,
	0x000b0000, 0x000e0000,
	0x00112000, 0x00160000, 0x001c0000, 0x00224000, 0x002c0000, 0x00380000,
	0x00448000, 0x00580000,
...
static void vepu541_h265_set_l2_regs(H265eV541HalContext * ctx,
				     H265eV54xL2RegSet * regs)
{
	MppEncHwCfg *hw = &ctx->cfg->hw;
	RK_U32 i;
	MppDevRegWrCfg cfg;

	memcpy(&regs->lvl32_intra_CST_THD0, lvl32_intra_cst_thd,
	       sizeof(lvl32_intra_cst_thd));
	memcpy(&regs->lvl16_intra_CST_THD0, lvl16_intra_cst_thd,
	       sizeof(lvl16_intra_cst_thd));
	memcpy(&regs->lvl32_intra_CST_WGT0, lvl32_intra_cst_wgt,
	       sizeof(lvl32_intra_cst_wgt));
	memcpy(&regs->lvl16_intra_CST_WGT0, lvl16_intra_cst_wgt,
	       sizeof(lvl16_intra_cst_wgt));
			...
			Vepu541Fmt input_fmt, HalEncTask * task)
{
	...
	regs->adr_srcu_hevc += u_offset;
	regs->adr_srcv_hevc += v_offset;
	return ret;
}
	...
	for (j = 0; j < mb_h; j++) {
		for (i = 0; i < mb_w; i++) {
			...
			for (cu16cnt = 0; cu16cnt < 16; cu16cnt++) {
				...
				cu16_y += j * 4;
				cu16_addr_in_frame =
				    cu16_x + cu16_y * cu16_num_line;
				dst[ctu_addr * 16 + cu16cnt] =
				    src[cu16_addr_in_frame];
			}
		}
	}
	return MPP_OK;
...
vepu541_h265_set_roi_regs(H265eV541HalContext * ctx, H265eV541RegSet * regs)
{
	...
	if (cfg->number && cfg->regions) {
		...
		if (!ctx->roi_hw_buf || roi_buf_size != ctx->roi_buf_size) {
			if (NULL == ctx->roi_grp) ;	// mpp_buffer_group_get_internal(&ctx->roi_grp, MPP_BUFFER_TYPE_ION);
			...
			if (NULL == ctx->roi_hw_buf) ;	// mpp_buffer_get(ctx->roi_grp, &ctx->roi_hw_buf, roi_buf_size);
...
static MPP_RET vepu541_h265_set_rc_regs(H265eV541HalContext * ctx,
					...
					HalEncTask * task)
{
	...
	ctu_target_bits_mul_16 = (rc_cfg->bit_target << 4) / (mb_wd64 * mb_h64);
...
static MPP_RET vepu541_h265_set_pp_regs(H265eV541RegSet * regs,
					VepuFmtCfg * fmt,
...
static void vepu541_h265_set_slice_regs(H265eSyntax_new * syn,
					H265eV541RegSet * regs)
{
...
static void vepu541_h265_set_ref_regs(H265eSyntax_new * syn,
				      H265eV541RegSet * regs)
{
...
static void vepu541_h265_set_me_regs(H265eV541HalContext * ctx,
				     H265eSyntax_new * syn,
				     H265eV541RegSet * regs)
{
	...
	{
		RK_S32 swin_scope_wd16 =
		    (regs->me_rnge.cime_srch_h + 3 + 1) / 4 * 2 + 1;
		...
		regs->me_ram.cime_rama_max =
		    (pic_wd64 * (tmpMin - 1)) +
		    ((pic_wd64 >=
		      swin_scope_wd16) ? swin_scope_wd16 : pic_wd64 * 2);
	}
...
static void vepu540_h265_set_me_ram(H265eSyntax_new * syn,
				    H265eV541RegSet * regs, RK_U32 index)
{
	RK_U32 cime_w = 11, cime_h = 7;
	RK_U32 pic_cime_temp = 0;
	if (syn->pp.tiles_enabled_flag == 0) {
		pic_cime_temp =
		    ((regs->enc_rsl.pic_wd8_m1 + 1) * 8 + 63) / 64 * 64;
		regs->me_ram.cime_linebuf_w = pic_cime_temp / 64;
	} else {
		RK_S32 pic_wd64 = MPP_ALIGN(syn->pp.pic_width, 64) >> 6;
		RK_S32 tile_ctu_stax =
		    index * pic_wd64 / (syn->pp.num_tile_columns_minus1 + 1);
		RK_S32 tile_ctu_endx = 0;
		RK_S32 cime_srch_w = regs->me_rnge.cime_srch_h;

		if (index == syn->pp.num_tile_columns_minus1) {
			tile_ctu_endx =
			    ((regs->enc_rsl.pic_wd8_m1 + 1) * 8 + 63) / 64 - 1;
		} else {
			tile_ctu_endx =
			    (index +
			     1) * pic_wd64 / (syn->pp.num_tile_columns_minus1 +
					      1) - 1;
		}

		if (tile_ctu_stax < (cime_srch_w + 3) / 4) {
			if (tile_ctu_endx + 1 + (cime_srch_w + 3) / 4 >
			    pic_wd64)
			...
			if (tile_ctu_endx + 1 + (cime_srch_w + 3) / 4 >
			    pic_wd64)
				pic_cime_temp =
				    (pic_wd64 - tile_ctu_stax +
				     (cime_srch_w + 3) / 4) * 64;
			else
				pic_cime_temp =
				    (tile_ctu_endx - tile_ctu_stax + 1 +
				     (cime_srch_w + 3) / 4 * 2) * 64;
		}
		regs->me_ram.cime_linebuf_w = pic_cime_temp / 64;
...

	{
		...
		while ((w_temp >
			((h_temp - h_val_0) * regs->me_ram.cime_linebuf_w * 4 +
			 ((h_val_1 - h_temp) * 4 * 7)))
		       && (h_temp < 17)) {
		...
		if (w_temp <
		    (RK_S32) ((h_temp -
			       h_val_0) * regs->me_ram.cime_linebuf_w * 4 +
			      ((h_val_1 - h_temp) * 4 * 7)))
			h_temp = h_temp - h_val_0;
	...
	hal_h265e_dbg_detail
	    ("cime_rama_h %d, cime_rama_max %d, cime_linebuf_w %d",
	     regs->me_ram.cime_rama_h, regs->me_ram.cime_rama_max,
	     regs->me_ram.cime_linebuf_w);
}

void vepu54x_h265_set_hw_address(H265eV541HalContext * ctx,
				 H265eV541RegSet * regs, HalEncTask * task)
{
	HalEncTask *enc_task = task;
	HalBuf *recon_buf = NULL, *ref_buf = NULL;
	...
	if (syn->pp.tiles_enabled_flag) {
		if (NULL == ctx->tile_grp)
			mpp_buffer_group_get_internal(&ctx->tile_grp,
						      MPP_BUFFER_TYPE_ION);

		mpp_assert(ctx->tile_grp);

		if (NULL == ctx->hw_tile_buf[0]) {
			mpp_buffer_get(ctx->tile_grp, &ctx->hw_tile_buf[0],
				       TILE_BUF_SIZE);
		}

		if (NULL == ctx->hw_tile_buf[1]) {
			mpp_buffer_get(ctx->tile_grp, &ctx->hw_tile_buf[1],
				       TILE_BUF_SIZE);
		}
	...
	regs->bsbt_addr_hevc =
	    regs->bsbb_addr_hevc + mpp_buffer_get_size(task->output) - 1;
...
MPP_RET hal_h265e_v541_gen_regs(void *hal, HalEncTask * task)
{
	...
	regs->enc_strt.rkvenc_cmd = 1;	//ctx->enc_mode;
	...
	regs->enc_pic.log2_ctu_num = mpp_ceil_log2(pic_wd64 * pic_h64);
	...
	if (ctx->is_vepu540) {
		regs->dtrns_cfg_540.cime_dspw_orsd =
		    (ctx->frame_type == INTER_P_FRAME);
		regs->dtrns_cfg_540.axi_brsp_cke = 0x0;
	} else {
		regs->dtrns_cfg_541.cime_dspw_orsd =
		    (ctx->frame_type == INTER_P_FRAME);
	...
	regs->src_proc.afbcd_en =
	    (MPP_FRAME_FMT_IS_FBC(syn->pp.mpp_format)) ? 1 : 0;

	regs->klut_ofst.chrm_kult_ofst =
...
void hal_h265e_v540_set_uniform_tile(H265eV541RegSet * regs,
				     H265eSyntax_new * syn, RK_U32 index)
{
	if (syn->pp.tiles_enabled_flag) {
		RK_S32 mb_w = MPP_ALIGN(syn->pp.pic_width, 64) / 64;
		RK_S32 mb_h = MPP_ALIGN(syn->pp.pic_height, 64) / 64;
		RK_S32 tile_width =
		    (index + 1) * mb_w / (syn->pp.num_tile_columns_minus1 + 1) -
		    index * mb_w / (syn->pp.num_tile_columns_minus1 + 1);
		if (index == syn->pp.num_tile_columns_minus1) {
			tile_width =
			    mb_w -
			    index * mb_w / (syn->pp.num_tile_columns_minus1 +
					    1);
		}
		...
		hal_h265e_dbg_detail
		    ("tile_x %d, rc_ctu_num %d, tile_width_m1 %d",
...
MPP_RET hal_h265e_v540_start(void *hal, HalEncTask * enc_task)
{
	...
	H265eV541IoctlOutputElem *reg_out =
	    (H265eV541IoctlOutputElem *) ctx->reg_out;
	H265eSyntax_new *syn = (H265eSyntax_new *) enc_task->syntax.data;
	RK_U32 title_num =
	    (syn->pp.num_tile_columns_minus1 +
	     1) * (syn->pp.num_tile_rows_minus1 + 1);
	RK_U32 stream_len = 0;
	...
	for (k = 0; k < title_num; k++) {	//v540 no support link list
		...
		/* set input info */
		vepu541_h265_set_l2_regs(ctx,
					 (H265eV54xL2RegSet *) ctx->l2_regs);
		if (title_num > 1)
		...
		if (k > 0) {
			...
			hw_regs->bsbb_addr_hevc =
			    mpp_dev_get_iova_address(ctx->dev, enc_task->output,
						     0);
					 ...
					 HalEncTask * enc_task)
{
	...
	H265eV541IoctlOutputElem *elem;
	RK_U32 hw_status;

	hal_h265e_enter();
	elem = (H265eV541IoctlOutputElem *) ctx->reg_out;
	hw_status = elem->hw_status;
	...
	memcpy(&fb->st_cu_num_qp[0], &elem->st_cu_num_qp[0],
	       sizeof(elem->st_cu_num_qp));
