...
//        mpp_assert(ctx->ext_line_buf_grp);
		...
		if (NULL == ctx->ext_line_buf)
			mpp_buffer_get(ctx->ext_line_buf_grp,
				       &ctx->ext_line_buf, ext_line_buf_size);
...
static RK_U32 update_vepu580_syntax(HalH264eVepu580Ctx * ctx,
				    MppSyntax * syntax)
{
	...
	if (slice->slice_type == H264_I_SLICE) {
		...
		if (!h264e_marking_is_empty(slice->marking)) {
			...
			do {
				...
				regs->reg_base.synt_refm2.long_term_frame_idx1 =
				    param_1;
				   ...
				   H264ePps * pps, H264eSlice * slice)
{
	...
	if (slice->slice_type == H264_I_SLICE) {
		...
		memcpy(&regs->reg_rc_klut.klut_wgt0, &h264e_klut_weight[0],
		       CHROMA_KLUT_TAB_SIZE);
	} else {
		regs->reg_rc_klut.klut_ofst.chrm_klut_ofst = 3;
		memcpy(&regs->reg_rc_klut.klut_wgt0, &h264e_klut_weight[4],
		       CHROMA_KLUT_TAB_SIZE);
	}
...
static void calc_cime_parameter(HalVepu580RegSet * regs, H264eSps * sps)
{
	...
	// calc cime_cacha_h and cime_cacha_max
	{
		...
		if (cime_cacha_max <
		    ((cime_cacha_h -
		      ctu_4_h) * base_regs->me_cach.cme_linebuf_w * 4 +
		...
		/* cime_cacha_max */
		{
			...
			base_regs->me_cach.cme_rama_max =
			    ram_col_h * base_regs->me_cach.cme_linebuf_w +
			    cur_srch_16_w;
		}
	}
}
			     ...
			     H264eSlice * slice)
{
	...
	if (cime_blk_h_max / 4 >
	    MPP_ALIGN(sps->pic_height_in_mbs * 16, 64) / 128 * 4)
		cime_blk_h_max =
		    MPP_ALIGN(sps->pic_height_in_mbs * 16, 64) / 128 * 16;
			     ...
			     MppEncHwCfg * hw)
{
	...
	if (slice->slice_type == H264_I_SLICE) {
		memcpy(regs->reg_s3.rdo_wgta_qp_grpa_0_51,
...
static void setup_vepu580_ext_line_buf(HalVepu580RegSet * regs,
...
static MPP_RET hal_h264e_vepu580_start(void *hal, HalEncTask * task)
{
	...
	do {
...
#if DUMP_REG
		{
			...
			for (i = 0;
			     i < sizeof(ctx->regs_set.reg_ctl) / sizeof(RK_U32);
			     i++) {
				/* code */
...
static MPP_RET hal_h264e_vepu580_ret_task(void *hal, HalEncTask * task)
{
	...
	rc_info->madi = (!ctx->regs_set.reg_st.st_bnum_b16.num_b16) ? 0 :
	    ctx->regs_set.reg_st.madi /
	    ctx->regs_set.reg_st.st_bnum_b16.num_b16;
	rc_info->madp =
	    (!ctx->regs_set.reg_st.st_bnum_cme.num_ctu) ? 0 : ctx->regs_set.
	    reg_st.madi / ctx->regs_set.reg_st.st_bnum_cme.num_ctu;
	rc_info->iblk4_prop =
	    (ctx->regs_set.reg_st.st_pnum_i4.pnum_i4 +
