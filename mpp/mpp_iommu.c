...
static struct mpp_dma_buffer *
mpp_dma_find_buffer(struct mpp_dma_session *dma, struct dma_buf *dmabuf)
{
	struct mpp_dma_buffer *out = NULL;
	struct mpp_dma_buffer *buffer = NULL, *n;
...
static struct mpp_dma_buffer *
mpp_iova_find_buffer(struct mpp_dma_session *dma, u32 iova)
{
	struct mpp_dma_buffer *out = NULL;
	struct mpp_dma_buffer *buffer = NULL, *n;
	mutex_lock(&dma->list_mutex);
...
int mpp_dma_release_dmabuf(struct mpp_dma_session *dma, struct dma_buf *dmabuf)
{
	struct device *dev = dma->dev;
	struct mpp_dma_buffer *buffer = NULL;

	buffer = mpp_dma_find_buffer(dma, dmabuf);
	if (IS_ERR_OR_NULL(buffer)) {
		dev_err(dev, "can not find %p buffer in list\n", dmabuf);
				      ...
				      struct dma_buf *dmabuf)
{
	...
	get_dma_buf(dmabuf);
	if (IS_ERR(dmabuf)) {
		mpp_err("dma_buf_get dmabuf %p failed\n", dmabuf);
		return NULL;
	}
	/* A new DMA buffer */
	mutex_lock(&dma->list_mutex);
	...
	mutex_lock(&dma->list_mutex);
	list_add_tail(&buffer->link, &dma->new_import_list);
	mutex_unlock(&dma->list_mutex);
...
mpp_dma_session_create(struct device *dev, u32 max_buffers)
{
	...
	INIT_LIST_HEAD(&dma->new_import_list);
